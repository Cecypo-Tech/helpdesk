...
.email-activity-card{
  /* index.css sets `color` on html/body using var(--ink-gray-9); that
     computed value is what descendants inherit. Redefining the custom
     property below doesn't retroactively recompute it, so unclassed text
     (e.g. the plain sender-name span) would still inherit the dark-mode
     color. Set `color` explicitly here so inheritance restarts fresh. */
  color: #171717;
}
